...
public class Dialog_ColourPicker : Window
{
    ...
    private void CreateAlphaBG(ref Texture2D bg, int width, int height)
    {
        ...
        var row = 0;
        for (var x = 0; x < width; x += _alphaBGBlockSize)
        {
            var column = row;
    ...
    private void CreateColourPickerBG()
    {
        ...
        var tex = new Texture2D(_pickerSize, _pickerSize);
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        var previewRect = new Rect(alphaRect.xMax + _margin, inRect.yMin, _previewSize, _previewSize);
        var previewOldRect = new Rect(previewRect.xMax, inRect.yMin, _previewSize, _previewSize);
        var doneRect = new Rect(alphaRect.xMax + _margin, inRect.yMax - _buttonHeight, _previewSize * 2,
            _buttonHeight);
        var setRect = new Rect(alphaRect.xMax + _margin, inRect.yMax - (2 * _buttonHeight) - _margin,
            _previewSize - (_margin / 2), _buttonHeight);
        var cancelRect = new Rect(setRect.xMax + _margin, setRect.yMin, _previewSize - (_margin / 2),
            _buttonHeight);
        var hsvFieldRect = new Rect(alphaRect.xMax + _margin,
            inRect.yMax - (2 * _buttonHeight) - (3 * _fieldHeight) - (4 * _margin),
            _previewSize * 2, _fieldHeight);
        var rgbFieldRect = new Rect(alphaRect.xMax + _margin,
            inRect.yMax - (2 * _buttonHeight) - (2 * _fieldHeight) - (3 * _margin),
            _previewSize * 2, _fieldHeight);
        var hexRect = new Rect(alphaRect.xMax + _margin,
            inRect.yMax - (2 * _buttonHeight) - (1 * _fieldHeight) - (2 * _margin),
            _previewSize * 2, _fieldHeight);
        var recentRect = new Rect(previewRect.xMin, previewRect.yMax + _margin, _previewSize * 2,
            _recentSize * 2);
        ...
        var hueHandleRect = new Rect(hueRect.xMin - 3f, hueRect.yMin + _huePosition - (_handleSize / 2f),
            _sliderWidth + 6f, _handleSize);
        var alphaHandleRect = new Rect(alphaRect.xMin - 3f, alphaRect.yMin + _alphaPosition - (_handleSize / 2f),
            _sliderWidth + 6f, _handleSize);
        var pickerHandleRect = new Rect(pickerRect.xMin + _position.x - (_handleSize / 2f),
            pickerRect.yMin + _position.y - (_handleSize / 2f), _handleSize, _handleSize);
